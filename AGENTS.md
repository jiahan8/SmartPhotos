...
Node.js Cloud Functions project in `functions/`. Twenty-nine Gradle modules, plus `build-logic/` — an
included build holding the eight convention plugins.
...
| `:core:common` | Android library, deliberately not Compose: the validation and failure strings + the mappers screens resolve them with (`validationErrorMessageResId`, `ErrorMessage.resolve`/`appErrorMessageResId`), `util/MediaUriExt.kt`, `NoteActionError.resolve`, and `note/di/NoteDelegateModule.kt`, which provides `:core:domain`'s two note delegates once per ViewModel (why it has Hilt/KSP). |
| `:core:data` | Android library holding every implementation of a `:core:domain`/`:core:common` contract that has not moved to a shared module below: the Android-bound repositories (`DefaultMediaFileRepository`, `DefaultAppUpdateRepository`), the Room and DataStore wiring (`DatabaseModule`, `DataStoreModule`), `FirebaseModule`, `DataModule`. |
...
| `:core:database` | Kotlin Multiplatform with an Android target, on `smartphotos.kmp.android.library`: Room's `AppDatabase`, `NoteDao`, `DatabaseNote` and `DatabaseConverters` in `commonMain`, the exported `schemas/`, and `NoteDaoTest`/`DatabaseConvertersTest` in `commonTest`. `:core:data`'s `DatabaseModule` opens it. |
| `:core:firebase` | Kotlin Multiplatform with an Android target, on `smartphotos.kmp.android.library` and GitLive's multiplatform Firebase SDK: the Firebase-backed repositories as they leave `:core:data` — `DefaultPhotoRepository`, `FirebaseRemoteConfigRepository`, `FirebaseAnalyticsRepository`, `DefaultAuthRepository`, `DefaultNoteRepository`, `DefaultUserRepository` and `DefaultMediaUploadRepository`, which is all of them — with their suites in `commonTest`. JVM 17, the build's one exception; iOS tests compile but do not link. `:core:data`'s `FirebaseModule`/`DataModule` wire it. |
...
| `:core:testing` | Shared Android test fixtures: `MainDispatcherRule`, and — re-exported via `api` — `:core:domain-testing`'s fakes. `testImplementation` only (plus `androidTestImplementation` wherever a `sharedTest/` runs in both). |
...
`:core:datastore`, `:core:database`, `:core:firebase` and each `:feature:<name>-viewmodel` use `src/commonMain/kotlin/`, with `:core:domain`'s `src/jvmMain/kotlin/` holding the one file that
...
`:core:datastore`/`:core:database`/`:core:firebase` → `:core:domain`; `:core:firebase` →
`:core:database`, for the Room mirror `DefaultNoteRepository` writes; `:app` → `:core:ui` →
...
**No feature depends on another feature, `:core:data`, `:core:datastore`, `:core:database` or `:core:firebase` — enforced at configuration time**, not
...
| Unit tests (706 across 25 modules) | `./gradlew testDebugUnitTest jvmTest` |
...
`:core:domain`, `:core:datastore`, `:core:database`, `:core:firebase` and the `:feature:<name>-viewmodel` modules are Kotlin Multiplatform, so their tests run under `jvmTest`, not the
...
- **Its tests live in `commonTest` and so must compile for every target**, which rules out
  ...
  **A backtick test name may not contain `,` either**: the JVM accepts it, Kotlin/Native rejects it
  with "Name contains illegal characters", and CI compiles no Apple target — so only a Mac's
  `iosSimulatorArm64Test` catches it.
...
- **`MigrationTestHelper` reads `schemas/` through the instrumentation context's assets** —
  `:core:database`'s `schemas/`, wired up in `core/data/build.gradle.kts` where the comment explains
  it. The failure mode when it isn't is "Cannot find the schema file in the assets folder" — nothing
  about migrations.
- **It stays in `:core:data` although `AppDatabase` is in `:core:database`, and should.** It pins
  the upgrade Android's framework SQLite performs, which is the one users run; the database
  module's `commonTest` runs Room's bundled SQLite instead. Its class doc gives the whole argument.
...
Eleven modules — `app/`, `core/data/`, and every `:feature:*`. 83 tests, and **CI runs all of
...
`src/sharedTest/kotlin`. Every feature's screen suite lives there, plus one in `:core:data`;
`:feature:auth` is the one to copy. **It is not only for Compose** —
`:core:data`'s `AppDatabaseMigrationTest` lives there too, because Robolectric supplies Android's
own SQLite, which is all it ever needed a device for. (`NoteDaoTest` and
`DefaultUserPreferencesRepositoryTest` were there as well, and left with their subjects for
`:core:database`'s and `:core:datastore`'s `commonTest` — on Room's bundled SQLite and on okio's
...
`settings.gradle.kts`), holding eight plugins that every module applies by id instead of restating the
same settings:
...
| `smartphotos.kmp.library` | `:core:domain`, `:core:domain-testing`, `:core:datastore`, `:core:database` and `:core:firebase` through `smartphotos.kmp.android.library`, and every `:feature:<name>-viewmodel` through `smartphotos.kmp.viewmodel` | Kotlin Multiplatform — **nothing Android** | `jvm()` + `iosArm64`/`iosSimulatorArm64` (no `iosX64`: `lifecycle-viewmodel` publishes none), Java 11, JVM target 11, test-JVM pin |
...
| `smartphotos.kmp.android.library` | `:core:database`, `:core:firebase` | `smartphotos.kmp.library`, AGP's `com.android.kotlin.multiplatform.library` | the Android target's `compileSdk`, `minSdk` and JVM target from `ProjectConfig` — for a shared module Android must consume as an Android build (Room's per-target codegen, GitLive's inline functions), not through its `jvm` variant |
...
- **`build-logic` targets Java 17, the modules target 11.** Not drift: the plugins run in the Gradle
  daemon (needs 17+), 11 is what the app compiles against. Don't "fix" either. **The one module
  exception is `:core:firebase`, at 17** because GitLive's inline functions are JVM 17 bytecode and
  Kotlin will not inline them into 11 — contained to the module that calls GitLive, and its build
  file says so. Don't raise any other module to match.
...
- **Repository** (`data/repository/`) — **one per data type, not one per feature**: media
  ...
  `:core:data` — except those already in `commonMain` (`DefaultUserPreferencesRepository` in
  `:core:datastore`, `DefaultPhotoRepository`, `FirebaseRemoteConfigRepository`,
  `FirebaseAnalyticsRepository`, `DefaultAuthRepository`, `DefaultNoteRepository`,
  `DefaultUserRepository` and `DefaultMediaUploadRepository` in `:core:firebase` on GitLive), which
  carry no annotations and so are constructed in `DataModule` providers rather than
  bound. **A Firebase repository leaving `:core:data` goes to `:core:firebase` on GitLive**, with its
  Firebase calls behind a small internal seam so its suite runs in `commonTest` — `UnsplashCallable`
  and `RemoteConfigSource` are the shape. **Where GitLive lacks an API the Android SDK has, don't
  drop the behaviour:** declare an `expect`, and have the Android `actual` reach the SDK through
  GitLive's own `android` accessor, as `configUpdates` does.
  **One interface can't live in `:core:domain` because its signatures carry Android types** —
  `AppUpdateRepository`, beside its implementation in `:core:data`; why is in
  [ARCHITECTURE.md](ARCHITECTURE.md#layers). Move it down only when a feature needs it.
- **Domain** (`domain/`, `:core:domain`) — plain data classes shared across features.
- **Local** — Room in `database/` (`:core:database`'s `commonMain`, schemas exported to
  `core/database/schemas/`, opened by `:core:data`'s `DatabaseModule`), DataStore in
  ...
  decodable. **A new per-user local store is registered with `data/DefaultLocalUserDataCleaner.kt` (`:core:data`, behind `:core:domain`'s `LocalUserDataCleaner`) in the
...
- **Keep `android.*` out of the *contracts*** — domain models, repository interfaces, and the data
  ...
  this targets; `AppUpdateRepository` is deliberately exempt, so don't cite it as precedent.
...
- **A ViewModel moves to `commonMain` the way `ExploreViewModel` did** — into its feature's own
  `<feature>-viewmodel` module on `smartphotos.kmp.viewmodel`, the class `open` and annotation-free,
...
resolution (`kotlin-serialization` in `:core:database`).
